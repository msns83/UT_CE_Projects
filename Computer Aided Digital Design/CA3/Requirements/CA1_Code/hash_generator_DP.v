module hase_generator_DP #(
    ...
    import_rom #(.DATA_WIDTH(WORD_SIZE), .ADDR_WIDTH(REPEAT_WIDTH), .MEM_FILE("constant.mem")) constant_rom (.addr(i_val), .data(constant_val));
    combinational_circuit #(WORD_SIZE, SELECTION_WIDTH) value_calc (.B(b), .C(c), .D(d), .range_sel(range_idx), .out(comb_output));
    stepRotator #(WORD_SIZE, REPEAT_WIDTH, SELECTION_WIDTH, 5) step_rotator(.in(rotate_input), .i(i_val), .range_sel(range_idx), .out(rotate_output));

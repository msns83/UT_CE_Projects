...
SELECT 
    DATE_FORMAT(create_at, '%Y-%m') AS Registration_Month, 
...
SELECT 
    SUBSTRING_INDEX(v.storage_path, '/', 3) AS Movie_Base_Path,
...
GROUP BY SUBSTRING_INDEX(v.storage_path, '/', 3)
...
SELECT 
    CASE 
        WHEN is_dubbed = TRUE THEN 'Dubbed' 
        WHEN is_subtitled = TRUE THEN 'Subtitled' 
    END AS Format,
...
SELECT 
    DAYNAME(create_at) AS Day_Of_Week, 

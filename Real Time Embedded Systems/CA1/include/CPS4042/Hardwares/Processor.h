...
public:
    ...
    ~Processor()
    {
        // Ensure the worker thread cannot outlive the processor / board.
        stopAndJoin();
    }
    ...
    {
        if (ProcessorBase::m_isBusy.load())
            return;
    ...
    nextCycle(Gpio &gpio)
    {
        ...
        m_isInitiated = true;
        nextCycleP(m_drivedCycle, exitCode, gpio);
    }

private:
    bool m_isInitiated{false};
    std::uint64_t m_drivedCycle{0};
    inline void

...
vector<string> Playlist ::getSongsListShort()
{
    ...
    result.push_back("ID, Name, Artist");
    for (auto &song : songs)
...
vector<string> Playlist ::getSongsListLong()
{
    ...
    result.push_back("ID, Name, Artist, Year, Album, Tags, Duration");
    for (auto &song : songs)
...
Song::Song(int _id, string _name, string _path, string _year, string _album, string _tags, string _duration, string _artist)
{
    id = _id;
    name = _name;
    path = _path;

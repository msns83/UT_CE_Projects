...
public class HighProteinMealPlanStrategy implements MealPlanStrategy {

    private final ProductServices productServices;
    private final CalorieEstimator calorieEstimator;
    private final PantryInventory pantryInventory;

    public HighProteinMealPlanStrategy(ProductServices productServices,
    ...
    private List<ProductScore> scoreProducts(List<Product> products, double targetPerMeal) {
        ...
        for (Product product : products) {
            ...
            double protein = extractProtein(product);
            double proteinDensity = (calories > 0) ? protein / calories : 0.0;
            double calorieDistance = Math.abs(targetPerMeal - calories);

            scored.add(new ProductScore(product, calories, proteinDensity, calorieDistance));
        }

        scored.sort((a, b) -> {
            int cmp = -Double.compare(a.proteinDensity, b.proteinDensity);
            if (cmp != 0) {
                return cmp;
            }
    ...
    private double extractProtein(Product product) {
        NutritionProfile profile = product.getNutritionProfile();
        if (profile == null) {
            return 0.0;
        }
        double proteinPerServing = profile.getProteinGrams();

...
public class BudgetFriendlyMealPlanStrategy implements MealPlanStrategy {

    private final ProductServices productServices;
    private final CalorieEstimator calorieEstimator;
    private final PantryInventory pantryInventory;

    public BudgetFriendlyMealPlanStrategy(ProductServices productServices,
    ...
    private List<ProductScore> scoreProducts(List<Product> products, double targetPerMeal) {
        ...
        for (Product product : products) {
            ...
            double price = Math.max(product.getPprice(), 0.01);
            double pricePerCalorie = (calories > 0) ? price / calories : Double.POSITIVE_INFINITY;
            double calorieDistance = Math.abs(targetPerMeal - calories);

            scored.add(new ProductScore(product, calories, pricePerCalorie, calorieDistance));
        }

        scored.sort((a, b) -> {
            int cmp = Double.compare(a.pricePerCalorie, b.pricePerCalorie);
            if (cmp != 0) {
                return cmp;
            }
            cmp = Double.compare(a.calorieDistance, b.calorieDistance);
            if (cmp != 0) {
                return cmp;
            }
    ...
    private static final class ProductScore {
        ...
        private final double pricePerCalorie;
        private final double calorieDistance;

        private ProductScore(Product product, double estimatedCalories,
                double pricePerCalorie, double calorieDistance) {
            this.product = product;
            this.estimatedCalories = estimatedCalories;
            this.pricePerCalorie = pricePerCalorie;
            this.calorieDistance = calorieDistance;
        }
    }
}
